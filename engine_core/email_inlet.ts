...
function loadConfig(): EmailConfig {
    ...
    return JSON.parse(fs.readFileSync(CONFIG_PATH, 'utf8')) as EmailConfig;
}
...
async function acquireAccessToken(config: EmailConfig): Promise<string> {
    const { tenant_id, client_id, client_secret } = config.m365;

    if (!tenant_id || !client_id || !client_secret) {
        throw new Error(
            'M365 credentials not configured. Set tenant_id, client_id, and client_secret in email_config.json.'
        );
    }
    ...
    const tokenData = (await response.json()) as M365TokenResponse;
...
async function fetchMessages(
    ...
    const receivedAfter = new Date(
        Date.now() - age_limit_hours * 60 * 60 * 1000
    ).toISOString();
    ...
    return (data.value ?? []) as M365Message[];
}

// ── Relevance filtering ──────────────────────────────────────────

function isRelevant(message: M365Message, config: EmailConfig): boolean {
    ...
    if (sender_allowlist.length > 0) {
        const allowed = sender_allowlist.some(
            allowed => senderEmail.toLowerCase().includes(allowed.toLowerCase())
        );
        if (!allowed) return false;
    }

    // Keyword match against subject and body preview
    const hasKeyword = subject_keywords.some(kw =>
        subject.includes(kw.toLowerCase()) || bodyPreview.includes(kw.toLowerCase())
    );
...
function messageToIngestPayload(message: M365Message): EmailIngestPayload {
...
function inferActionFromEmail(message: M365Message): string {
...
function writeToIngest(payload: EmailIngestPayload, messageId: string): string {
...
export async function runEmailIngestion(): Promise<{
    ...
    try {
        ...
        console.error(`❌ [EMAIL INLET] Authentication failed: ${(err as Error).message}`);
        return { fetched: 0, relevant: 0, ingested: 0, skipped: 0 };
    }

    // Step 2: Fetch messages from inbox
    let messages: M365Message[];
    try {
        messages = await fetchMessages(accessToken, config, syncState);
    } catch (err) {
        console.error(`❌ [EMAIL INLET] Failed to fetch messages: ${(err as Error).message}`);
        return { fetched: 0, relevant: 0, ingested: 0, skipped: 0 };
    }
