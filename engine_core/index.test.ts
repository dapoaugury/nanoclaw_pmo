...
// Import types and constants from index.ts (we'll test the logic directly)
...
class TestEngine {
    ...
    constructor() {
        ...
        if (fs.existsSync(this.tempDbPath)) fs.unlinkSync(this.tempDbPath);
        if (fs.existsSync(this.tempDbPath + '-wal')) fs.unlinkSync(this.tempDbPath + '-wal');
        if (fs.existsSync(this.tempDbPath + '-shm')) fs.unlinkSync(this.tempDbPath + '-shm');
    ...
    public scoreActionRisk(payload: IngestPayload): RiskFinding[] {
    ...
    public scoreParameterRisk(payload: IngestPayload): RiskFinding[] {
        const findings: RiskFinding[] = [];
    ...
    public scoreFinancialRisk(payload: IngestPayload): RiskFinding[] {
        const findings: RiskFinding[] = [];
    ...
    public scoreGraphConflicts(payload: IngestPayload, graph: Array<{entity: string; relationship: string; target: string}>): RiskFinding[] {
        const findings: RiskFinding[] = [];
    ...
    public aggregateRisk(findings: RiskFinding[]): number {
    ...
    public actionRequiresApproval(payload: IngestPayload | null): boolean {
        if (!payload?.requested_action) return false;
        return ALWAYS_INTERVENE_ACTIONS.has(payload.requested_action.toUpperCase());
    }

    public evaluateInterventionGate(riskScore: number, findings: RiskFinding[], payload: IngestPayload | null): boolean {
        return (
            riskScore >= 0.5 ||
...
try {
    ...
    assert(engine.actionRequiresApproval({ requested_action: 'EXECUTE_BUDGET_RECONCILIATION' }) === true,
        'EXECUTE_BUDGET_RECONCILIATION requires approval');
    assert(engine.actionRequiresApproval({ requested_action: 'deploy_production' }) === true,
        'DEPLOY_PRODUCTION (case insensitive) requires approval');

    // Test 7.2: Non-approval actions
    assert(engine.actionRequiresApproval({ requested_action: 'UPDATE_TASK' }) === false,
        'Regular action does not require approval');
    ...
    const budgetPayload: IngestPayload = {
        task_id: 'JIRA-789',
    ...
    const safePayload: IngestPayload = {
        task_id: 'JIRA-100',
    ...
    const dangerousPayload: IngestPayload = {
        task_id: 'JIRA-999',
